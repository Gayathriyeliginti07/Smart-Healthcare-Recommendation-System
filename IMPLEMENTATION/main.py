...
dataset = pd.read_csv(os.path.join('datasets', 'Training.csv'))
...
sym_des = pd.read_csv(os.path.join("datasets", "symtoms_df.csv"))
...
doctors = pd.read_csv(os.path.join("datasets", "doctors.csv"))
...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction', 
            33: 'Peptic ulcer diseae', 1: 'AIDS', 12: 'Diabetes ', 17: 'Gastroenteritis', 6: 'Bronchial Asthma', 23: 'Hypertension ',
            30: 'Migraine', 7: 'Cervical spondylosis', 32: 'Paralysis (brain hemorrhage)', 28: 'Jaundice', 29: 'Malaria',
            8: 'Chicken pox', 11: 'Dengue', 37: 'Typhoid', 40: 'hepatitis A', 19: 'Hepatitis B', 20: 'Hepatitis C', 21: 'Hepatitis D', 
...
def helper(dis):
    try:
        desc = description.loc[description['Disease'] == dis, 'Description'].iloc[0]
        pre = precautions.loc[precautions['Disease'] == dis, ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.flatten().tolist()
        med = medications.loc[medications['Disease'] == dis, 'Medication'].to_list()
        die = diets.loc[diets['Disease'] == dis, 'Diet'].to_list()
        wrkout = workout.loc[workout['disease'] == dis, 'workout'].to_list()
        return desc, pre, med, die, wrkout
    except Exception as e:
        logging.error(f"Data missing for disease '{dis}': {e}")
...
def get_predicted_value(patient_symptoms):
    ...
    common_symptoms = ['headache', 'fatigue', 'cough', 'nausea', 'fever']
    common_diseases = {
        'headache': 'Migraine',
    ...
    if len(patient_symptoms) == 1 and patient_symptoms[0] in common_symptoms:
        return common_diseases.get(patient_symptoms[0], predicted_disease)
...
def predict():
    ...
    dis_des, my_precautions, meds, my_diet, workout_plan = helper(predicted_disease)
...
def get_doctors():
    ...
    specialist = doctors.loc[doctors['Disease'] == disease, 'Doctor Specialist'].values
    if specialist.size == 0:
        return jsonify({'error': 'No specialist found for this disease'})

    return jsonify({'specialist': specialist[0]})
